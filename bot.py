...
@bot.command(name="format", help="Learn how to format answers")
async def format(ctx):
    ...
    await ctx.send("If an answer does not have any digits but 0 when rounded to 4 places, enter '0.0'")
...
async def probq(ctx):
    ...
    dice_problems = {'the number on one die is bigger than that on the other?': '0.8333',
        ...
        'a number I choose between 2 and 12 (with equal weight) is at least the sum of the two dice?':
        '0.5455', 'each die has a Fibonacci number, given the sum is a Fibonacci number?': '0.5833'}
    ...
    card_problems = {'a card is drawn. What is the probability it is the Ace of Spades?': '0.0192',
        '3 cards are drawn without replacement. What is the probability they are all spades?': '0.0129',
        '3 cards are drawn without replacement. What is the probability exactly 2 are of the same suit?':
        '0.5506', '5 cards are drawn without replacement. What is the probability 4 have the same ' + 
        'number, given at least 3 have the same number?': '0.0105'}
...
async def miscprobq(ctx):
    ...
    problems = {f'I have {f_i + s_i + t_i} cards: {f_i} are red ' + 
        ...
        f'side and red on the other. I choose a random card and see it is red on this side. What is ' +
        f'the probability the other side is blue?': f'{round(t_i / (2 * f_i + t_i), 4)}',
        ...
        f'not taking the bus is {round(1 - f_f - s_f + t_f, 2)}, find the probability it is raining, given ' +
        f'David is not taking the bus.': f'{round((s_f - t_f) / (1 - f_f), 4)}',
...
async def distq(ctx):
    ...
    f'I roll a fair {c_i}-sided die until I get a two {b_i} times. Find the standard deviation of the ' +
    f'number of rolls.': f'{round(math.sqrt(b_i * (1 - (1 / c_i)) / ((1 / c_i) ** 2)), 4)}',
    ...
    f'{round(e_i * ((1 / d_i) ** 2), 4)}', f'Zoe receives phone calls according to a Poisson ' +
    f'process with a rate of {g_t} calls per hour. Find the probability it takes more than {h_i} minutes ' +
    f'for her to receive her first one.': f'{round(math.exp(-1 * g_t * h_i / 60), 4)}',
    f'Zoe receives phone calls according to a Poisson process with a rate of {g_t} calls per hour. ' +
    f'Find the probability it takes less than {h_i} minutes for her to receive her first one.':
    f'{round(1 - math.exp(-1 * g_t * h_i / 60), 4)}',
    f'Zoe receives phone calls according to a Poisson process with a rate of 4 calls per hour. ' +
    f'Find the probability it takes between 1 and 2 hours for her to receive her sixth one.':
    f'0.5939', f'Let X~N{d_i, h_i}. P[X<{a_i}] is equivalent to P[Z<a]. Find a.':
